...
public class AlarmController {
    ...
    public ResponseEntity<Alarm> confirmAlarm(@PathVariable Long alarmId) {
        Alarm updatedAlarm = alarmService.confirmAlarm(alarmId);
        return ResponseEntity.ok(updatedAlarm); // 수정된 알람을 포함하여 200 OK 반환
    }

    // 타임캡슐에 대한 알람 생성 API
    @PostMapping("/{timeCapsuleId}")
    public ResponseEntity<Alarm> createAlarmForTimeCapsule(
            @PathVariable Long timeCapsuleId,
            @RequestParam String title,
            @RequestParam String content) {
        Alarm createdAlarm = alarmService.createAlarm(title, content, timeCapsuleId);
